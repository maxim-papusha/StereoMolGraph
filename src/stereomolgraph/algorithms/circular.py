...
def _reaction_generator(
    ...
    for _ in counter:
        for axis, it in enumerate(color_iters):
            color = next(it)
            assert len(color) == n_atoms, (
                f"len(color): {len(color)}, n_atoms: {n_atoms}, axis: {axis}"
            )
            if stacked is None:
                stacked = np.empty((*color.shape, 3), dtype=np.int64)
...
def modulo_fold(vec: np.ndarray, n_bits: int = 1024, count: bool = True) -> np.ndarray:
...
def circular_stereo_fingerprint(
    graph: StereoMolGraph,
    radius: int = 3,
    n_bits: int = 2048,
    count: bool = True,
