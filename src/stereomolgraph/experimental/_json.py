...
STEREO_CLASSES: dict[str, type] = {
    ...
    "AtropBond": AtropBond,
    "RigidBond12": RigidBond12,
    "RigidBond13": RigidBond13,
    "RigidBond23": RigidBond23,
    "RigidBond33": RigidBond33,
}
...
def external_symmetry_number(graph: StereoMolGraph) -> int:
    ...
    counter: dict[BondAutoCls, dict[frozenset[RigidBond33], set[MappingId]]] = {}

    for bond_auto_cls, bonds in int_bond_auto_set.items():
        counter[bond_auto_cls] = {}

        bond_to_state: dict[Bond, set[RigidBond33]] = {}
        ...
        for bond in bonds:
            ...
            if (
                ...
                nrb_set.add(RigidBond33(atoms=(*left, a1, a2, *right), parity=0))
    ...
    for mapping_id, mapping in enumerate(mappings):
        for bond_auto_cls, bond_state_dict in counter.items():
            for bond_state, mapping_ids in bond_state_dict.items():
                if all(
                    RigidBond33(tuple(mapping[a] for a in non_rot_bond.atoms), parity=0)
                    in bond_state
